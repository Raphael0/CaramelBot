...
while True:
    ...
    for y in range(gameMap.height):
        for x in range(gameMap.width):
            if gameMap.getSite(Location(x, y)).owner == RandomBot:
                movedPiece = False
                for d in CARDINALS:
                    if gameMap.getSite(Location(x, y), d).owner != RandomBot and gameMap.getSite(Location(x, y), d).strength < gameMap.getSite(Location(x, y)).strength:
